...
strain <- read_delim(file = "UCI HAR Dataset/train/subject_train.txt", col_names = FALSE, delim = '')
...
# fixed limits don't work with subject_train files (it only reads the first character)
stest <- read_delim(file = "UCI HAR Dataset/test/subject_test.txt", col_names = FALSE, delim = '')

merged <- rbind(cbind(ytrain, strain, Xtrain), cbind(ytest, stest, Xtest))
